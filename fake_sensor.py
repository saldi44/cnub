...
TEMPERATURA_MAX = 50.00
TEMPERATURA_MIN = -50.00
HUMEDAD_MAX = 100
HUMEDAD_MIN = 0
...
if __name__ == "__main__":
    ...
    if ( args.tempMax ):
        temp_min=args.tempMax
    if ( args.tempMin ):
        temp_max=args.tempMin
    if ( args.humMax ):
        hum_max=args.humMax
    if ( args.humMin ):
        hum_min=args.humMin

    logger.debug("Iniciando sensor")
    ...
    medidor_temperatura = Gauge( "temperatura", DESCRIPCION, labelnames=['instance', 'lon', 'lat',
    'unidades'], registry=registro)

    medidor_humedad = Gauge( "humedad", DESCRIPCION, labelnames=['instance', 'lon', 'lat',
    'unidades'], registry=registro)
